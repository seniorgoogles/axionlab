...
class Jsc(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.name == "jsc_xl":
            ...
            x = self.relu2(self.dense2(x.to(torch.float32)))
            x = self.relu3(self.dense3(x.to(torch.float32)))
            x = self.relu4(self.dense4(x.to(torch.float32)))
            x = self.dense5(x.to(torch.float32))
